...
Maximus::Maximus(const std::string& name, const std::string& type, int strength,
                 Weapon& weapon, Energy* energy,
                 const std::string vulnerability, int speed)
    : Transformer(name, type, strength, weapon, energy),
      vulnerability(vulnerability),
      speed(speed) {}

std::string Maximus::getVulnerability() const { return vulnerability; }

void Maximus::setVulnerability(const std::string newVulnerability) {
  vulnerability = newVulnerability;
}

int Maximus::getSpeed() const { return speed; }

void Maximus::setSpeed(int newSpeed) { speed = newSpeed; }
...
std::string Maximus::display() const {
  return "Maximus. Name: " + getName() + ", Type: " + getType() +
